(() => {
    ...
    function initSliders() {
        if (document.querySelector(".main-page__slider")) new core(".main-page__slider", {
            ...
            navigation: {
                prevEl: ".swiper-button-prev",
                nextEl: ".main-page__slider .swiper-button-next"
            },
            on: {}
        });
    }
    ...
    if (mouse) {
        function moveMouse(e) {
            if (e.target.closest(".projects-body__item")) {
                mouse.classList.add("active");
                mouse.style.transform = `translate(-50%, -50%) translate(${e.clientX - 0}px, ${e.clientY - 0}px)`;
            } else mouse.classList.contains("active") ? mouse.classList.remove("active") : null;
            if (e.clientX < 5 || e.clientY < 5 || e.clientX > window.innerWidth - 5 || e.clientY > window.innerHeight - 5) mouse.classList.contains("active") ? mouse.classList.remove("active") : null;
        }
        if (window.innerWidth >= 768) addEventListener("mousemove", moveMouse);
    }
    window["FLS"] = true;
    isWebp();
    menuInit();
